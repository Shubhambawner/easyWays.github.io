...
function changeIndtruct(q, flag) {
    $("#" + q).css("display", "block"),
    "2" == q ? ($("#1").css("display", "none"),
    $("#3").css("display", "none")) : "1" == q ? ($("#2").css("display", "none"),
    $("#3").css("display", "none")) : "3" == q ? ($("#1").css("display", "none"),
    $("#2").css("display", "none")) : ($("#2").css("display", "none"),
    $("#3").css("display", "none")),
    null != q && 0 != flag && $.ajax({
        method: "GET",
        async: !1,
        ...
        $(".ulInstruction> li:visible").length > 1 && $(".ulInstruction li:last-child").remove(),
        "" != data && $.each(JSON.parse(data.Instructions), (function(i, item) {
...
function check_instruction(id) {
    0 == $("#" + id + "_ch").prop("checked") ? "1" == id ? swal({
        ...
        confirmButtonClass: "btn-primary",
        showLoaderOnConfirm: !0
    }) : window.location.href = $("#hdfBaseUrl").val() + "nta/ntaclone/Paper.html" 
}
function page_click() {
    $(document).on("click", "#btnStart", (function(e) {
        e.preventDefault();
        var frm = $("#frmexamtype"), frmParsley, btn;
...
function fill_paper(drp, ID) {
    $.ajax({
        ...
        data.List.length > 0 && $.each(data.List, (function(index, value) {
